...
## In this file — What this creature IS
...
**20. A real animal keeps its real name; invented names are for invented creatures.** Call a crocodile a crocodile, a jackal a jackal, a wolf a wolf. An evocative name on a creature the reader already knows ("Marsh Lurker" for a crocodile) is a promise the stat block cannot keep — the table braces for something strange and gets an animal — and it spends a good name on the one creature that needed no help being understood. Qualifiers that carry real mechanical information are fine and often necessary (`Young Crocodile`, `Dire Hyena`), because they place the creature on its lineage's tier ladder. **Save invented names for Magical Beasts, Horrors, Spirits and Divine Beasts**, where the name is the reader's only clue what they are looking at. **When a family spans tiers, the mythological name belongs to the HIGHER-tier member** and the lower
rungs take plain descriptive names (D-109). The tier-1 undead is the **Grave Husk** and *edimmu* is saved
for the incorporeal shade at tier 4. A tier-1 mook called by a myth-name spends the myth on the first
encounter of a campaign, and D-068 already says rank-and-file names are generic. Sets the pattern for
gallu, the serpents, and every other reused name. Corollary: a good invented name that gets used on an ordinary animal is gone — "Marsh Lurker" is now reserved for a pseudo-animal that is genuinely not of this world. *(Owner ruling, 2026-08-09.)*
