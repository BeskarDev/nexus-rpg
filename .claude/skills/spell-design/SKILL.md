...
**Numeric tables (rank scaling, ranges, areas, deviations) live in [references/rank-scaling.md](references/rank-scaling.md). School identities live in [references/schools.md](references/schools.md). Use them exactly — don't design from memory.** Core system mechanics and writing standards: [../game-basics.md](../game-basics.md).
...
When the owner corrects or refines a design decision in session, append it to [references/designer-principles.md](references/designer-principles.md) (numbered, one line, with the reasoning) and, if it's frequently load-bearing, add it to the short list in the Designer Principles section above. That file is the accumulated design memory — it must grow. Keep SKILL.md itself lean: new lessons and reference material go into `references/` files.
