...
- **Rank 4 — Master (encounter-shaping)**: broader scope, tactical decisions. Templates: scope expansion (single→multi-target), action economy (Action→Quick Action), reliability (conditional→consistent), system bridge (combat→Challenge/Travel), recovery (mitigate→reverse), earned recharge (a fitting in-fiction trigger — a kill, a checkpoint, a milestone — refills an earlier rank's scarce once-per-day/once-per-adventure resource, deepening it rather than adding a new unrelated effect; principle 33). Numbers may grow within the §4.5 menu ceilings, but never as the rank's whole content — a rank that is JUST a number bump is boring and uninspired (owner ruling); lead with a new capability, let the menu step ride along.
...
**Beyond these structural rules, the binding design-judgment rules live in [references/designer-principles.md](references/designer-principles.md)** — 18 principles ported from the extensively-developed spell-design corpus plus talent-native additions, and pointers to the shared spell-design wording/GM-facing/condition convention files that apply to talent text verbatim. Read it before any design pass. The most frequently load-bearing: talents assist, never bypass (1); flat damage riders don't proliferate — prefer action economy, control, banes/boons (2); SL manipulation is premium R3+ budget, never stacked with a flat rider (3); never scale a cap (4); a granted immunity needs a circumvention (5); debuffs key off rolls that actually happen (8); high-impact conditions never ride no-roll triggers (9); GM-facing effects give bounded parameters, never "GM decides" (14); weigh worldbuilding reach of networks/forgery/information talents (15); cross-system interactions gate in the talent itself (17); validate against spells and items, not just talents (18); system vocabulary (Motivation/Pitfall/Interest/Patience) is a payoff, not a trigger gate — only live numeric resources need an active-challenge gate (27); Challenges are a resolution overlay a GM lays over a scene, never a dependency — describe the narrative action first with a baseline payoff that works in any scene, then layer challenge-die manipulation as a pace-gated alternate (28); ground a genuine narrative-fact term (Motivation, Pitfall) with its own concrete examples on first use (29); procedural bookkeeping labels (Safepoint, Blockade, Encounter, Weather Event) get natural lowercase language instead of a capital letter, since they're the GM's filing category, not something the fiction contains (30); a single ability should basically never impose +2 banes — the second bane adds a further flat -1 on top of the first's take-lower-2d6 swing, making it a control-tier effect; match a peer's actual mechanic (a flat Defense bonus) or spread difficulty across time/coin/scope instead (32); a talent must earn its slot across multiple game systems, not one low-screen-time system — travel, research, social, and crafting talents each need a dungeon-delving or combat application on at least one rank, with the narrow-system ability as one facet, not the whole kit (combat-only is the sole exception); check the skill's own pool for the idiom and audit for overlap (35).
