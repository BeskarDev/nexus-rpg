---
sidebar_position: 0
---

# Quickstart Characters

{/* GENERATED from src/utils/data/json/archetypes.json by `bun run content:gen` — do not edit. Edit the JSON and regenerate. */}

![banner-img](/img/banner/character-creation-banner.png)

> "The greatest warriors were once novices with nothing but determination and a well-chosen path."

## Using These Examples

These quickstart characters demonstrate classic fantasy archetypes within Nexus RPG. Each example follows the standard character creation rules:

- **Starting XP:** 6 XP total (3 skills at rank 1 with 2 XP each, 4 skills at rank 0)
- **Starting Coins:** 350 coins for equipment (plus 10 unspendable coins)
- **Standard Gear:** Backpack, 2 pouches, traveler's clothes, hemp rope (1 load), camping kit (1 load), toolkit (1 load), rations d4 (1 load), torches d4 (1 load)
  - If an archetype lists a specific toolkit, it is the one from Standard Gear: it costs 0 extra coins and its load is already counted in the 5-load Standard Gear.
  - Selecting a weapon that needs ammunition gives you one unit of that ammunition for free.

Each archetype includes recommended attributes, skill selections with talent choices, upbringing and background, equipment purchases within the 350 coin budget, and playstyle descriptions.

Feel free to customize these examples to match your vision or use them as-is for quick character creation.

## Choose a Role

Every archetype leans on one job in a party. Start here, then compare the details in the table below.

### Tank

- [Champion](/docs/basic-rules/quickstart-characters/champion) - Holy warrior, heavy armor
- [Fighter](/docs/basic-rules/quickstart-characters/fighter) - Reliable frontline combat
- [Hoplite](/docs/basic-rules/quickstart-characters/hoplite) - Formation fighting, spear

### Striker

- [Apothecary](/docs/basic-rules/quickstart-characters/apothecary) - Bombs, potions, devices
- [Barbarian](/docs/basic-rules/quickstart-characters/barbarian) - Massive damage, rage mechanics
- [Brawler](/docs/basic-rules/quickstart-characters/brawler) - Unarmed, grapples, dirty tricks
- [Duelist](/docs/basic-rules/quickstart-characters/duelist) - Agile precision melee
- [Gladiator](/docs/basic-rules/quickstart-characters/gladiator) - Arena control, takedowns
- [Magus](/docs/basic-rules/quickstart-characters/magus) - Melee + magic hybrid
- [Monk](/docs/basic-rules/quickstart-characters/monk) - Unarmed combat, grappling
- [Ranger](/docs/basic-rules/quickstart-characters/ranger) - Ranged combat, wilderness
- [Rogue](/docs/basic-rules/quickstart-characters/rogue) - Stealth, infiltration, skills
- [Slinger](/docs/basic-rules/quickstart-characters/slinger) - Mobile ranged skirmisher
- [Sorcerer](/docs/basic-rules/quickstart-characters/sorcerer) - Devastating offensive magic
- [Swashbuckler](/docs/basic-rules/quickstart-characters/swashbuckler) - Dirty tactics, dual-wielding
- [Warlock](/docs/basic-rules/quickstart-characters/warlock) - Dark magic, mind control

### Support

- [Bard](/docs/basic-rules/quickstart-characters/bard) - Performance magic, social
- [Druid](/docs/basic-rules/quickstart-characters/druid) - Nature magic, shapeshifting
- [Engineer](/docs/basic-rules/quickstart-characters/engineer) - Siege, traps, devices
- [Oracle](/docs/basic-rules/quickstart-characters/oracle) - Prophecy, divination, fate
- [Priest](/docs/basic-rules/quickstart-characters/priest) - Healing, divine magic, melee
- [Shaman](/docs/basic-rules/quickstart-characters/shaman) - Spirit communion, ancestors
- [Summoner](/docs/basic-rules/quickstart-characters/summoner) - Conjuration, minions, battlefield control
- [Tamer](/docs/basic-rules/quickstart-characters/tamer) - Animal handler, battlefield control
- [Warlord](/docs/basic-rules/quickstart-characters/warlord) - Battlefield commander

## Archetypes at a Glance

| Archetype | Role | Primary Skills | Best For |
|-----------|------|----------------|----------|
| [Apothecary](/docs/basic-rules/quickstart-characters/apothecary) | Striker / Utility | Archery, Crafting, Nature | Bombs, potions, devices |
| [Barbarian](/docs/basic-rules/quickstart-characters/barbarian) | Striker / Tank | Fighting, Athletics, Fortitude | Massive damage, rage mechanics |
| [Bard](/docs/basic-rules/quickstart-characters/bard) | Support / Controller | Mysticism, Influence, Education | Performance magic, social |
| [Brawler](/docs/basic-rules/quickstart-characters/brawler) | Striker / Controller | Fighting, Fortitude, Streetwise | Unarmed, grapples, dirty tricks |
| [Champion](/docs/basic-rules/quickstart-characters/champion) | Tank / Support | Fighting, Mysticism, Influence | Holy warrior, heavy armor |
| [Druid](/docs/basic-rules/quickstart-characters/druid) | Support / Controller | Mysticism, Nature, Survival | Nature magic, shapeshifting |
| [Duelist](/docs/basic-rules/quickstart-characters/duelist) | Striker | Fighting, Athletics, Stealth | Agile precision melee |
| [Engineer](/docs/basic-rules/quickstart-characters/engineer) | Support / Utility | Crafting, Education, Archery | Siege, traps, devices |
| [Fighter](/docs/basic-rules/quickstart-characters/fighter) | Tank / Striker | Fighting, Athletics, Fortitude | Reliable frontline combat |
| [Gladiator](/docs/basic-rules/quickstart-characters/gladiator) | Striker / Controller | Fighting, Athletics, Influence | Arena control, takedowns |
| [Hoplite](/docs/basic-rules/quickstart-characters/hoplite) | Tank / Controller | Fighting, Athletics, Influence | Formation fighting, spear |
| [Magus](/docs/basic-rules/quickstart-characters/magus) | Striker / Hybrid | Arcana, Fighting, Education | Melee + magic hybrid |
| [Monk](/docs/basic-rules/quickstart-characters/monk) | Striker / Controller | Fighting, Athletics, Fortitude | Unarmed combat, grappling |
| [Oracle](/docs/basic-rules/quickstart-characters/oracle) | Support / Utility | Mysticism, Lore, Insight | Prophecy, divination, fate |
| [Priest](/docs/basic-rules/quickstart-characters/priest) | Support / Tank | Mysticism, Fighting, Lore | Healing, divine magic, melee |
| [Ranger](/docs/basic-rules/quickstart-characters/ranger) | Striker / Utility | Archery, Survival, Nature | Ranged combat, wilderness |
| [Rogue](/docs/basic-rules/quickstart-characters/rogue) | Striker / Utility | Fighting, Stealth, Streetwise | Stealth, infiltration, skills |
| [Shaman](/docs/basic-rules/quickstart-characters/shaman) | Support / Controller | Mysticism, Nature, Lore | Spirit communion, ancestors |
| [Slinger](/docs/basic-rules/quickstart-characters/slinger) | Striker | Archery, Athletics, Stealth | Mobile ranged skirmisher |
| [Sorcerer](/docs/basic-rules/quickstart-characters/sorcerer) | Striker | Arcana, Lore, Education | Devastating offensive magic |
| [Summoner](/docs/basic-rules/quickstart-characters/summoner) | Support / Controller | Arcana, Lore, Insight | Conjuration, minions, battlefield control |
| [Swashbuckler](/docs/basic-rules/quickstart-characters/swashbuckler) | Striker / Controller | Fighting, Stealth, Streetwise | Dirty tactics, dual-wielding |
| [Tamer](/docs/basic-rules/quickstart-characters/tamer) | Support / Controller | Fighting, Nature, Athletics | Animal handler, battlefield control |
| [Warlock](/docs/basic-rules/quickstart-characters/warlock) | Striker / Controller | Arcana, Lore, Insight | Dark magic, mind control |
| [Warlord](/docs/basic-rules/quickstart-characters/warlord) | Support / Controller | Fighting, Influence, Education | Battlefield commander |

## Customization Tips

**Adjusting Attributes**
While each example shows recommended attributes, you can swap the d8 and d4 to create different flavors:

- **Agile Fighter:** d8 AGI, d4 MND. Focus on light armor and quick weapons.
- **Strong Archer:** d8 STR, d4 MND. Use heavy bows and thrown weapons.

**Skill Variations**
The 3 rank 1 skills define your starting capabilities, but the 4 rank 0 skills show growth potential. Different rank 0 skills support varied secondary roles. Consider your party composition when selecting skills.

**Equipment Priorities**
With 350 coins, prioritize based on your role:

- **Frontline:** Invest in weapons and armor.
- **Casters:** Get your catalyst (included in toolkit), save coins for consumables.
- **Utility:** Balance between combat gear and tools.

**Talent Flexibility**
Each rank 1 skill offers multiple talent options. Consider offensive talents for strikers, defensive talents for tanks, and utility talents for support roles.

Remember: these are starting points. Your character will grow and specialize as you gain XP and advance through the ranks!
