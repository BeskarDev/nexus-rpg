# Ability Tags: Per-Ability Audit Data

This document holds the full per-ability tag assignment data produced during the tag audit (Phase 2 of the migration). It is the input data for the implementation spec at `spec/ability-tags-implementation.md`, which describes how these assignments move into the canonical content JSON files. The tag taxonomy, application guidelines, and design rationale live in [ability-tags.md](ability-tags.md).
