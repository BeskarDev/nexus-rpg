...
| Illusory Dodge | 1 | Defense / standalone | Standardized R1 Quick Action reactive defense (principle 4): +2 to Dodge or Parry against the triggering attack. School secondary: if the attack misses, the attacker was fooled by the flickering double and is briefly distracted. Weak-role entry mandated by internal completeness (principle 9). |
...
| Shattered Veil | 2 | Offense / payoff | Castable only on a creature currently deceived by one of your active illusory spells: the lie collapses violently into a psychic burst, dealing damage (ignore AV) and leaving the target briefly confused. Cold-cast deficit is absolute (no valid target without an active illusion). The school's first mechanical spend of the believed state. |
...
| Phantasmal Catastrophe | 4 | Offense / payoff | A mass hallucination of disaster (the sky falling, the earth splitting) in a medium area: below-curve AoE psychic damage cold, but any target currently frightened, confused, or deceived by one of your illusory spells takes damage one step higher and is frightened. Spends the school's whole condition suite at battlefield scale; cold-cast deficit explicit. |
...
- [x] R1 Quick Action: Illusory Dodge (R1 seed) — standardized reactive defense (+2 Dodge/Parry, illusory double secondary)
