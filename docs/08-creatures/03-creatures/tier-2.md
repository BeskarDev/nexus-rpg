---
sidebar_position: 3
---

# Tier 2

Tier 2 creatures are slightly more dangerous, including stronger undead, minor magical beings, and organized humanoid fighters.

---

### **Zombie** (Medium Undead)

**Tier:** 2 (Basic)
**Armor:** Light

| HP | AV | STR | AGI | SPI | MND | Parry | Dodge | Resist |
|----|----|----|----|----|-----|-------|-------|--------|
| 20 | 2 (natural light) | d8 | d4 | d4 | d4-1 | 8 | 6 | 8 |

**Skills:** Fortitude (2), Fighting (1)

**Weaknesses:** slash weapons, radiant damage

**Resistances:** pierce weapons, necrotic damage

**Immunities:** bleeding, charmed, confused, deafened, frightened, poisoned, unconscious

**Attacks:**

- **Claw**. 6/10/14 damage. On a hit, attempts to grapple the target.
- **Rotten Bite** (*only if grappling*). 8/14/20 poison damage.

**Abilities:**

- **Undead Nature (Passive).** This creature is immune to bleeding, being charmed, confused, deafened, frightened, poisoned, or falling unconscious.
- **Rotting Slowness (Passive).** This creature can only move or take an Action per turn, not both. This creature can't use Quick Actions.

---

### **Plague Zombie** (Medium Undead)

**Tier:** 2 (Basic)
**Armor:** Light

| HP | AV | STR | AGI | SPI | MND | Parry | Dodge | Resist |
|----|----|----|----|----|-----|-------|-------|--------|
| 20 | 2 (natural light) | d8 | d4 | d4 | d4-1 | 8 | 6 | 8 |

**Skills:** Fortitude (2), Fighting (1)

**Weaknesses:** slash weapons, radiant damage

**Resistances:** pierce weapons, necrotic damage, poison damage

**Immunities:** bleeding, charmed, confused, deafened, frightened, poisoned, unconscious

**Attacks:**

- **Claw**. 6/10/14 damage. On a hit, attempts to grapple the target.
- **Rotten Bite** (*only if grappling*). 8/14/20 poison damage.
- **Spew Plague** (*thrown (short)*). 8/14/20 poison damage. On a strong or critical hit, the target must roll Strength + Fortitude vs. TN 8 or become diseased.

**Abilities:**

- **Undead Nature (Passive).** This creature is immune to bleeding, being charmed, confused, deafened, frightened, poisoned, or falling unconscious.
- **Rotting Slowness (Passive).** This creature can only move or take an Action per turn, not both. This creature can't use Quick Actions.

---

### **Orc Warrior** (Medium Humanoid)

**Tier:** 2 (Basic)
**Armor:** Light

| HP | AV | STR | AGI | SPI | MND | Parry | Dodge | Resist |
|----|----|----|----|----|-----|-------|-------|--------|
| 20 | 2 (light) | d8 | d6 | d6 | d4 | 8 | 8 | 8 |

**Skills:** Athletics (2), Fighting (2), Fortitude (2)

**Attacks:**

- **Battleaxe** (*slash, versatile +1*). 8/12/16 damage.

**Abilities:**

- **Orcish Fury (Passive).** When this creature hits with a melee weapon and rolls a critical success, add its weapon damage an additional time to the total damage.

---

### **Lizardfolk Hunter** (Medium Humanoid)

**Tier:** 2 (Basic)
**Armor:** Light

| HP | AV | STR | AGI | SPI | MND | Parry | Dodge | Resist |
|----|----|----|----|----|-----|-------|-------|--------|
| 20 | 3 (natural light) | d6 | d8 | d6 | d4 | 8 | 9 | 8 |

**Skills:** Archery (1), Athletics (2), Fighting (1), Fortitude (2), Perception (2), Survival (2)

**Attacks:**

- **Bite** (*crush*). 7/11/15 damage.
- **Club** (*agile, crush, light*). 6/10/14 damage.
- **Javelin** (*bundle, light, thrown (medium)*). 6/10/14 damage.
- **Net** (*bundle, entangle, heavy (d6), thrown (short)*). On a hit, the target is restrained until they escape (Strength + Athletics vs. TN 8).

**Abilities:**

- **Aquatic Nature (Passive).** This creature can hold its breath for 4 + 1/2 Strength minutes and can swim without any penalty to its Movement.

---

### **Lizardfolk Raider** (Medium Humanoid)

**Tier:** 2 (Basic)
**Armor:** Light

| HP | AV | STR | AGI | SPI | MND | Parry | Dodge | Resist |
|----|----|----|----|----|-----|-------|-------|--------|
| 20 | 3 (natural light) | d8 | d6 | d6 | d4 | 9 | 8 | 8 |

**Skills:** Athletics (2), Fighting (2), Fortitude (2), Perception (2), Survival (2)

**Attacks:**

- **Bite** (*crush*). 8/12/16 damage.
- **War Club** (*crush, versatile +1*). 8/12/16 damage.

**Abilities:**

- **Aquatic Nature (Passive).** This creature can hold its breath for 4 + 1/2 Strength minutes and can swim without any penalty to its Movement.

---

### **Lizardfolk Tamer** (Medium Humanoid)

**Tier:** 2 (Basic)
**Armor:** Light

| HP | AV | STR | AGI | SPI | MND | Parry | Dodge | Resist |
|----|----|----|----|----|-----|-------|-------|--------|
| 20 | 3 (natural light) | d8 | d6 | d6 | d4 | 8 | 8 | 8 |

**Skills:** Fighting (1), Fortitude (2), Nature (2), Perception (2), Survival (2)

**Attacks:**

- **Bite** (*crush*). 8/12/16 damage.
- **Whip** (*agile, entangle, light, reach, slash*). 6/10/14 damage.

**Abilities:**

- **Aquatic Nature (Passive).** This creature can hold its breath for 4 + 1/2 Strength minutes and can swim without any penalty to its Movement.
- **Beast Tamer (Passive).** This creature can control one large sized animal, two medium sized ones, or three small sized ones at a time. The creature(s) act under this creature's control as long as they can see and hear them.

**Quick Actions:**

- **Command Beast.** A creature under this creature's control that can see and hear them can re-roll one test.

---

### **Dark Cultist Acolyte** (Medium Humanoid)

**Tier:** 2 (Basic)
**Armor:** Light

| HP | AV | STR | AGI | SPI | MND | Parry | Dodge | Resist |
|----|----|----|----|----|-----|-------|-------|--------|
| 20 | 2 (light) | d6 | d6 | d6 | d8 | 8 | 8 | 9 |

**Skills:** Arcana (2), Influence (2), Lore (2), Perception (2)

**Attacks:**

- **Ritual Dagger** (*agile, light, pierce*). 6/10/14 damage.
- **Dark Bolt** (*range (medium)*). Roll Mind + Arcana. 6/10/14 necrotic damage. On a strong or critical hit, the target suffers +1 bane on their next roll.

**Abilities:**

- **Innate Spellcasting (Passive).** This creature can cast the following spells rolling Mind + Arcana once per scene each: *Attack Thoughts*, *Subtle Suggestion*.
- **Dark Devotion (Passive).** This creature gains +1 boon on saves against being charmed or frightened.

---

### **Dark Cultist Assassin** (Medium Humanoid)

**Tier:** 2 (Basic)
**Armor:** Light

| HP | AV | STR | AGI | SPI | MND | Parry | Dodge | Resist |
|----|----|----|----|----|-----|-------|-------|--------|
| 20 | 2 (light) | d8 | d8 | d6 | d6 | 8 | 9 | 8 |

**Skills:** Archery (2), Fighting (2), Perception (2), Stealth (2)

**Attacks:**

- **Poisoned Dagger** (*agile, light, pierce*). 6/10/14 damage. On a strong or critical hit, the target is briefly poisoned.
- **Shortbow** (*ammo, range (medium), two-handed*). 6/10/14 damage.

**Abilities:**

- **Sneak Attack (Passive).** This creature deals +4 damage against surprised targets or targets that are engaged with an ally.
- **Evasion (Passive).** When this creature would take damage from an area attack, it can roll Agility + Athletics. On a success, it takes half damage. On a strong or critical success, it takes no damage.

**Quick Actions:**

- **Vanish.** If this creature is in dim light or darkness, it can use a Quick Action to become hidden until the end of its next turn or until it attacks.

---

### **Draugr** (Medium Undead)

**Tier:** 2 (Basic)
**Armor:** Heavy

| HP | AV | STR | AGI | SPI | MND | Parry | Dodge | Resist |
|----|----|----|----|----|-----|-------|-------|--------|
| 20 | 4 (natural heavy) | d8 | d4 | d6 | d4 | 9 | 7 | 8 |

**Skills:** Fighting (2), Fortitude (2), Perception (1)

**Weaknesses:** fire damage, radiant damage

**Resistances:** necrotic damage, frost damage

**Immunities:** bleeding, charmed, frightened, poisoned, unconscious

**Attacks:**

- **Rusted Axe** (*slash*). 8/12/16 damage. On a strong or critical hit, the target suffers bleeding (2).
- **Chilling Grasp** (*target: vs. Resist*). 6/10/14 frost damage. On a strong or critical hit, the target is slowed until the end of their next turn.

**Abilities:**

- **Undead Nature (Passive).** This creature is immune to bleeding, being charmed, frightened, poisoned, or falling unconscious.
- **Grave Chill (Passive).** Any creature that starts its turn within melee range of this creature takes 2 frost damage.

---

### **Scarab Swarm** (Medium Beast)

**Tier:** 2 (Basic)
**Armor:** Light

| HP | AV | STR | AGI | SPI | MND | Parry | Dodge | Resist |
|----|----|----|----|----|-----|-------|-------|--------|
| 20 | 2 (natural light) | d4 | d8 | d4 | d4-2 | 7 | 9 | 8 |

**Skills:** Fighting (1), Perception (2), Stealth (2)

**Resistances:** physical damage (from single-target attacks)

**Immunities:** conditions from single-target effects

**Attacks:**

- **Devouring Bites**. 6/10/14 damage. On a strong or critical hit, the target's AV is reduced by 1 until they spend a Quick Action to brush off the scarabs.

**Abilities:**

- **Swarm (Passive).** This swarm consists of many individual creatures that act in unison. The swarm is resistant to any damage from effects that target a single creature and immune to conditions from such effects.
- **Flesh Eaters (Passive).** This swarm deals +2 damage against creatures with no armor or natural armor.

---

### **Medium Fire Elemental** (Medium Spirit)

**Tier:** 2 (Basic)
**Armor:** Light

| HP | AV | STR | AGI | SPI | MND | Parry | Dodge | Resist |
|----|----|----|----|----|-----|-------|-------|--------|
| 20 | 2 (natural light) | d8 | d8 | d8 | d6 | 8 | 9 | 8 |

**Skills:** Fighting (2), Fortitude (2), Perception (2)

**Weaknesses:** frost damage, damage from water

**Resistances:** physical damage

**Immunities:** fire damage, burning, bleeding, poisoned, grappled, restrained, stunned, unconscious

**Attacks:**

- **Fast Strikes.** This creature can attack or cast spells twice per turn.
- **Burning Lash**. 6/10/14 fire damage. On a strong or critical hit, inflicts burning (4) for a short duration.
- **Flame Bolt** (*range (medium)*). 6/10/14 fire damage. On a strong or critical hit, inflicts burning (2) for a short duration.

**Abilities:**

- **Ethereal Form (Passive).** This creature is immune to bleeding, poison damage, being poisoned, being grappled, restrained, stunned, or falling unconscious.
- **Flame Body (Passive).** This creature is immune to fire damage and burning. This creature illuminates the area in short distance around it in bright light. If this creature is hit by an attack at melee range, the attacker suffers 4 fire damage (ignoring AV). If this creature comes into contact with a large amount of water, it takes 5 damage (ignoring AV) each turn doing so.

---

### **Goblin Boss** (Small Humanoid)

**Tier:** 2 (Lord)
**Armor:** Light

| HP | AV | STR | AGI | SPI | MND | Parry | Dodge | Resist |
|----|----|----|----|----|-----|-------|-------|--------|
| 3x20 | 3 (light) | d4 | d8 | d4 | d6 | 7 | 9 | 8 |

**Skills:** Archery (1), Athletics (2), Fighting (2), Perception (2), Stealth (2)

**Attacks:**

- **Fast Strikes.** This creature can attack twice per turn.
- **Scimitar** (*agile, light, slash*). 6/10/14 damage.
- **Whip** (*agile, entangle, light, reach, slash*). 6/10/14 damage.
- **Dart** (*agile, bundle, light, pierce, thrown (short)*). 6/10/14 damage.

**Abilities:**

- **Night Vision (Passive).** This creature can see one range category further from sources of bright and dim light.
- **Rally Goblin (Action).** On this creature's turn, it can target one goblin visible in medium range. That goblin immediately makes an attack against one target of this creature's choice. The goblin gains +1 boon on the attack. This ability can be used once per turn.
- **Quick Escape (Passive).** When this creature was missed by a melee attack, it can move close.
- **Goblin Reinforcements (Lord Trigger).** When this creature loses a life pool, it summons 1d4+2 goblins, which appear at any location in short range of this creature.

**Quick Actions:**

- **Sacrifice Goblin.** When this creature would be hit by an attack, it can redirect the attack to target one goblin close to them.

---

### **Veteran Soldier** (Medium Humanoid)

**Tier:** 2 (Basic)
**Armor:** Heavy

| HP | AV | STR | AGI | SPI | MND | Parry | Dodge | Resist |
|----|----|----|----|----|-----|-------|-------|--------|
| 20 | 4 (heavy + helmet + shield) | d8 | d8 | d6 | d6 | 9 | 8 | 8 |

**Skills:** Athletics (2), Archery (2), Education (2), Fighting (2), Perception (2)

**Attacks:**

- **Fast Strikes.** This creature can attack twice per turn.
- **Spear** (*pierce, reach, versatile +1*). 7/11/15 damage.
- **Broadsword** (*slash, versatile +1*). 7/11/15 damage.
- **Light Shield** (*AV +1, parry +1*). 5/7/9 damage.
- **Longbow** (*ammo, heavy (d6), range (long), two-handed*). 7/11/15 damage.

**Abilities:**

- **Versatile Fighter (Passive).** This creature can choose to either add +1 boon, deal +2 damage per SL, make a second attack with +1 bane, make the target fall prone (melee only), or disarm the target (melee only) on any attack it makes. It can choose any one option only once per combat.

**Quick Actions:**

- **Second Wind.** Once per combat, this creature can use its Quick Action on its turn to regain 8 HP.

---

### **Dire Wolf** (Large Beast)

**Tier:** 2 (Basic)
**Armor:** Light

| HP | AV | STR | AGI | SPI | MND | Parry | Dodge | Resist |
|----|----|----|----|----|-----|-------|-------|--------|
| 20 | 3 (natural light) | d10 | d6 | d6 | d4-1 | 9 | 7 | 8 |

**Skills:** Fighting (2), Fortitude (2), Perception (2), Survival (2)

**Attacks:**

- **Bite**. 9/15/21 damage. On a hit, attempts to grapple the target. On a strong or critical hit against a creature of smaller size, the target is knocked prone.

**Abilities:**

- **Keen Scent (Passive).** This creature gains +1 boon on Perception rolls based on smell.
- **Pack Tactics (Passive).** While in a troop or in melee range of allies, this creature's attacks gain +1 boon.

---

### **Crocodile** (Large Beast)

**Tier:** 2 (Basic)
**Armor:** Light

| HP | AV | STR | AGI | SPI | MND | Parry | Dodge | Resist |
|----|----|----|----|----|-----|-------|-------|--------|
| 20 | 3 (natural light) | d10 | d6 | d4 | d4-1 | 9 | 7 | 8 |

**Skills:** Fighting (2), Fortitude (2), Perception (2), Survival (2)

**Attacks:**

- **Bite** (*crush*). 9/15/21 damage. On a strong or critical hit, the target is grappled.
- **Tail Swipe** (*reach*). 7/11/15 damage. On a strong or critical hit, the target falls prone.

**Abilities:**

- **Amphibious (Passive).** This creature can breathe both in air and water.
- **Death Roll (Action).** If this creature has a target grappled, it can use its Action to automatically deal 9 damage and stun the target until the end of its next turn.

---

### **Ghoul** (Medium Undead)

**Tier:** 2 (Basic)
**Armor:** Light

| HP | AV | STR | AGI | SPI | MND | Parry | Dodge | Resist |
|----|----|----|----|----|-----|-------|-------|--------|
| 20 | 2 (natural light) | d8 | d8 | d6 | d4 | 8 | 9 | 8 |

**Skills:** Fighting (2), Fortitude (2), Perception (2), Stealth (2)

**Weaknesses:** radiant damage

**Resistances:** necrotic damage

**Immunities:** bleeding, charmed, poisoned, unconscious

**Attacks:**

- **Claws** (*agile*). 6/10/14 damage. On a strong or critical hit, the target must roll Strength + Fortitude vs. TN 8 or be paralyzed for a short duration.
- **Bite** (*crush*). 8/14/20 damage.

**Abilities:**

- **Undead Nature (Passive).** This creature is immune to bleeding, being charmed, poisoned, or falling unconscious.
- **Corpse Eater (Passive).** This creature regains 5 HP when it starts its turn within melee range of a dead creature.

---

### **Animated Armor** (Medium Automaton)

**Tier:** 2 (Basic)
**Armor:** Heavy

| HP | AV | STR | AGI | SPI | MND | Parry | Dodge | Resist |
|----|----|----|----|----|-----|-------|-------|--------|
| 20 | 4 (natural heavy) | d8 | d6 | d4 | d4-2 | 9 | 7 | 8 |

**Skills:** Fighting (2)

**Immunities:** bleeding, charmed, confused, frightened, poisoned, unconscious

**Attacks:**

- **Longsword** (*pierce, versatile +1*). 8/12/16 damage.

**Abilities:**

- **Construct Nature (Passive).** This creature is immune to bleeding, being charmed, confused, frightened, poisoned, or falling unconscious.
- **Antimagic Susceptibility (Passive).** This creature is incapacitated while in the area of an antimagic field or similar effect.
- **False Appearance (Passive).** While this creature remains motionless, it is indistinguishable from a normal suit of armor.

---

### **Hobgoblin Warrior** (Medium Humanoid)

**Tier:** 2 (Basic)
**Armor:** Heavy

| HP | AV | STR | AGI | SPI | MND | Parry | Dodge | Resist |
|----|----|----|----|----|-----|-------|-------|--------|
| 20 | 4 (heavy + shield) | d8 | d6 | d6 | d6 | 9 | 7 | 8 |

**Skills:** Athletics (2), Fighting (2), Fortitude (2), Perception (2)

**Attacks:**

- **Longsword** (*pierce, versatile +1*). 8/12/16 damage.
- **Longbow** (*ammo, heavy (d6), range (long), two-handed*). 8/12/16 damage.

**Abilities:**

- **Martial Advantage (Passive).** Once per turn, this creature can deal an extra 4 damage to a creature it hits with a weapon attack if that creature is within melee range of an ally.
- **Formation Fighting (Passive).** While adjacent to an ally, this creature gains +1 Parry.

---

### **Gray Ooze** (Medium Ooze)

**Tier:** 2 (Basic)
**Armor:** Light

| HP | AV | STR | AGI | SPI | MND | Parry | Dodge | Resist |
|----|----|----|----|----|-----|-------|-------|--------|
| 20 | 2 (natural light) | d8 | d4 | d4 | d4-2 | 8 | 6 | 8 |

**Skills:** Fighting (1), Stealth (2)

**Resistances:** acid, cold, fire damage

**Immunities:** lightning damage, blinded, charmed, deafened, frightened, poisoned, prone, unconscious

**Attacks:**

- **Pseudopod**. 6/10/14 acid damage. In addition, nonmagical armor worn by the target is partly dissolved. The armor takes a permanent and cumulative −1 penalty to AV. If the penalty reduces its AV to 0, the armor is destroyed.

**Abilities:**

- **Amorphous (Passive).** This creature can move through a space as narrow as a crack without squeezing.
- **Corrode Metal (Passive).** Any nonmagical metal weapon that hits this creature corrodes. After dealing damage, the weapon takes a permanent and cumulative −1 penalty to damage rolls.
- **False Appearance (Passive).** While this creature remains motionless, it is indistinguishable from an oily pool or wet rock.

---

### **Vine Blight** (Medium Plant)

**Tier:** 2 (Basic)
**Armor:** Light

| HP | AV | STR | AGI | SPI | MND | Parry | Dodge | Resist |
|----|----|----|----|----|-----|-------|-------|--------|
| 20 | 3 (natural light) | d10 | d4 | d6 | d4-1 | 9 | 6 | 8 |

**Skills:** Fighting (2), Fortitude (2), Stealth (2)

**Immunities:** blinded, deafened

**Attacks:**

- **Constrict** (*crush*). 7/11/15 damage. On a hit, the target is grappled. Until this grapple ends, the target is restrained, and this creature can't constrict another target.
- **Entangling Vines** (*range (short)*). The area within close range of a point this creature can see becomes difficult terrain for a short duration. Each creature in the area when the effect begins must roll Strength + Athletics vs. TN 8 or be restrained by the vines until they escape.

**Abilities:**

- **False Appearance (Passive).** While this creature remains motionless, it is indistinguishable from a tangle of vines.
- **Regeneration (Passive).** This creature regains 5 HP at the start of its turn if it is in bright sunlight and has at least 1 HP.

---

### **Twig Blight** (Small Plant)

**Tier:** 2 (Basic)
**Armor:** Light

| HP | AV | STR | AGI | SPI | MND | Parry | Dodge | Resist |
|----|----|----|----|----|-----|-------|-------|--------|
| 15 | 2 (natural light) | d6 | d8 | d4 | d4-1 | 7 | 9 | 8 |

**Skills:** Fighting (1), Stealth (2)

**Weaknesses:** fire damage

**Immunities:** blinded, deafened

**Attacks:**

- **Claw** (*agile*). 5/7/9 damage.

**Abilities:**

- **False Appearance (Passive).** While this creature remains motionless, it is indistinguishable from a dead shrub.
- **Pack Tactics (Passive).** While in a troop or in melee range of allies, this creature's attacks gain +1 boon.

---

### **Myconid Sprout** (Small Plant)

**Tier:** 2 (Basic)
**Armor:** Light

| HP | AV | STR | AGI | SPI | MND | Parry | Dodge | Resist |
|----|----|----|----|----|-----|-------|-------|--------|
| 15 | 2 (natural light) | d6 | d6 | d8 | d6 | 7 | 8 | 8 |

**Skills:** Nature (2), Perception (2)

**Immunities:** blinded, deafened

**Attacks:**

- **Fist** (*crush*). 5/7/9 damage.

**Abilities:**

- **Distress Spores (Passive).** When this creature takes damage, all other myconids within medium range can sense its pain.
- **Rapport Spores (Action).** This creature releases a cloud of spores. All creatures within close range that aren't undead, constructs, or elementals can communicate telepathically with each other for a short duration.
- **Sun Sickness (Passive).** While in sunlight, this creature has +1 bane on all rolls.
