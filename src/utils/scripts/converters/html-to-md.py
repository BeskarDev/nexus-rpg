...
def html_table_to_dataframe(html_file):
  ...
  Returns:
      pandas.DataFrame: The DataFrame containing the data from the HTML table, or None if no table found.
  """
  # Read the HTML file with fallback encoding
  try:
  ...
  # Find the table element - prefer collection-content table if it exists (Notion database view)
  table = soup.find('table', class_='collection-content')
  if table is None:
...
if __name__ == "__main__":
  ...
  parser.add_argument("input_path", help="Path to input HTML file or directory")
  parser.add_argument("output_path", nargs="?", help="Path to output markdown file (for single file mode)")
  ...
  if os.path.isfile(input_path):
      if not args.output_path:
          print("Error: Output path required for single file mode")
          exit(1)
      
      # Convert file to DataFrame
      df = html_table_to_dataframe(input_path)
      ...
      df_to_markdown(df, output_path)
      print(f"Successfully converted {os.path.basename(input_path)} to {output_path}")
  ...
  # Directory mode
      ...
      for filename in os.listdir(input_path):
          ...
          if os.path.isfile(input_file):
              ...
              # Skip files that don't contain tables
              if df is None:
