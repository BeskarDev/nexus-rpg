...
export const ArcaneSpells: React.FC = () => {
  const [selectedArcaneSpells, setSelectedArcaneSpells] = React.useState<string[]>([]);

  const handleChange = (event: SelectChangeEvent<typeof selectedArcaneSpells>) => {
    const {
      target: { value },
    } = event;
  ...
  const selectAll = () => setSelectedArcaneSpells(arcaneSpells.map(ca => ca.name))
  ...
  return (
    <ThemeProvider theme={theme}>
      ...
      <Typography variant="subtitle1">{filteredArcaneSpells.length} Arcane Spells will be printed:</Typography>
      <div className="arcane-spell--container" ref={componentRef}>
        ...
        {!filteredArcaneSpells.length && <Typography variant="body2">Select some Arcane Spells above to include them for printing.</Typography>}
      </div>
    </ThemeProvider>
  );
};
