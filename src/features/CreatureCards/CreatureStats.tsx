...
const getStatsFontSize = (
	...
	if (attacks && abilities) {
		...
		const abilityContent = abilities
			...
			.join(' ')
		const totalContent =
			skillsContent + defensiveContent + attackContent + abilityContent
...
export const CreatureStats: React.FC<CreatureStatsProps> = ({
	...
	const textFontSize = getStatsFontSize(
		skills,
		immunities,
		resistances,
		weaknesses,
		...
		<>
			...
			>
				...
				<Box className="stat-item">
					...
					<Typography sx={{ fontSize: '7pt !important' }}>
						{simplifiedAV}
					</Typography>
				</Box>
			</Box>
			...
			{skills.length > 0 && (
				<Typography
					sx={{
						mb: 0.4,
						fontSize: `${textFontSize} !important`,
						lineHeight: 1.2,
					}}
				>
