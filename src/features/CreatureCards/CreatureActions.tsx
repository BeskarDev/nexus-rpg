...
export const CreatureActions: React.FC<CreatureActionsProps> = ({
  ...
  return (
    <>
      ...
      {attacks.length > 0 && (
        <Box sx={{ mb: 0.3 }}>
          <Typography sx={{ fontSize: '6.5pt !important', fontWeight: 600, mb: 0.25 }}>
            Attacks
          </Typography>
          {attacks.map((attack, index) => (
            <Box key={index} sx={{ mb: 0.3 }}>
              <Typography sx={{ fontSize: `${fontSize} !important`, mb: 0.1, lineHeight: 1.2 }}>
              ...
              {attack.description && (
                <Typography sx={{ fontSize: `${fontSize} !important`, mb: 0.1, lineHeight: 1.2, opacity: 0.9 }}>
                  {attack.description}
                </Typography>
              )}
            </Box>
      ...
      {abilities.length > 0 && (
        <Box sx={{ mb: 0.5 }}>
          ...
          {abilities.map((ability, index) => (
            <Typography key={index} sx={{ fontSize: `${fontSize} !important`, mb: 0.2, lineHeight: 1.2 }}>
