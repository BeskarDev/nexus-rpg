...
export const personalTabSchema = yup.object({
	...
	folk: yup.string().max(100, 'Must not exceed 100 characters'),

	upbringing: yup.string().max(100, 'Must not exceed 100 characters'),

	background: yup.string().max(100, 'Must not exceed 100 characters'),

	motivation: yup.string().max(100, 'Must not exceed 100 characters'),

	height: yup.string().max(30, 'Must not exceed 30 characters'),

	weight: yup.string().max(30, 'Must not exceed 30 characters'),

	age: yup.string().max(20, 'Age text is too long'),

	description: yup
		.string()
		.max(1000, 'Description must not exceed 1000 characters'),

	notes: yup.string().max(5000, 'Notes must not exceed 5000 characters'),
})
 ...
 * @returns Yup schema with dynamic max validation
 */
export const createSkillXpSchema = (
	totalSpentXp: number,
	currentSkillXp: number = 0,
) => {
	...
	const spentXpExcludingCurrentSkill = Math.max(
		0,
		totalSpentXp - currentSkillXp,
	)
