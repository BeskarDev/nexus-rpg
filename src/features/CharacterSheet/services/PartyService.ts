...
export class PartyService {
	...
	static async createParty(
		...
		try {
			...
			try {
				...
				throw new Error(
					`Failed to update character: ${updateError.message || 'Unknown error'}`,
				)
			}
	 ...
	 */
	static async addCharacterToParty(
		partyId: string,
		characterId: string,
	): Promise<void> {
		...
		if (!party.members.includes(characterId)) {
			...
			if (existingPartyInfo) {
				throw new Error(
					'Character is already in a party. They must leave their current party first before joining a new one.',
				)
			}
	 ...
	 */
	static async removeCharacterFromParty(
		partyId: string,
		characterId: string,
	): Promise<void> {
		try {
			...
			if (!collectionId || !docId) {
				throw new Error(
					'Invalid character ID format - expected format: "collectionId-docId"',
				)
			}
	...
	static subscribeToParty(
		partyId: string,
		callback: (partyInfo: PartyInfo | null) => void,
	): Unsubscribe {
		return onSnapshot(
	...
	static async deleteParty(
		...
		try {
			...
			if (party.members.length !== 1 || !party.members.includes(characterId)) {
				throw new Error(
					'You can only delete a party if you are the only member',
				)
			}
			...
			if (!characterDoc.exists()) {
				throw new Error(
					'Character not found - cannot update character document',
				)
			}
	 ...
	 */
	static async getPartyByCharacterId(
		characterId: string,
