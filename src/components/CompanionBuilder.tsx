...
const SIZE_MODIFIERS: Record<string, { parry: number; dodge: number; movement: number }> = {
...
export const CompanionBuilder: React.FC = () => {
  ...
  const modifyDie = (baseDie: string, modifier: string): string => {
    ...
    const diceOrder = ['d4-2', 'd4-1', 'd4', 'd6', 'd8', 'd10', 'd12', 'd12+1', 'd12+2'];
    const baseIndex = diceOrder.indexOf(baseDie);
    const modValue = parseInt(modifier);
    
    if (isNaN(modValue)) return baseDie;
    
    const newIndex = Math.max(0, Math.min(diceOrder.length - 1, baseIndex + modValue));
  ...
  const parseAttackDamage = (attackText: string, baseAttackDamage: { weak: number; normal: number; strong: number }, tier: number): string => {
    ...
    if (useDamageCalculation) {
      ...
      processedText = processedText.replace(/Deals .*?weapon damage.*?\./g, '').trim();
      ...
      const propertiesMatch = processedText.match(/(<strong>.*?<\/strong>)\s*(\(<em>.*?<\/em>\))/);
      if (propertiesMatch) {
    ...
    processedText = processedText.replace(/(\d+)\s*x\s*Tier/gi, (match, multiplier) => {
  ...
  const formatSkillsWithRanks = (skillsText: string, baseSkillRank: number): string => {
  ...
  const calculateStats = (tier: number, size: string, trait: CompanionTrait) => {
    ...
    // Calculate Defenses
    const defenses = {
      parry: baseStats.defenses.parry + sizeModifier.parry + applyModifier(0, trait.parry),
      dodge: baseStats.defenses.dodge + sizeModifier.dodge + applyModifier(0, trait.dodge),
    ...
    return {
      ...
      abilities: [trait['ability 1'], trait['ability 2'], trait['ability 3']].filter(ability => ability && ability !== '-'),
    };
  };
  ...
  const buildCompanion = () => {
    ...
    const calculatedStats = calculateStats(selectedTier, selectedSize, selectedTrait);
  ...
  const generateMarkdown = (companion: CompanionStats) => {
    ...
    const markdownAttacks = calculatedStats.attacks.map(attack => `- ${convertHtmlToMarkdown(attack)}`).join('\n');
    const markdownAbilities = calculatedStats.abilities.map(ability => `- ${convertHtmlToMarkdown(ability)}`).join('\n');
              ...
              <Grid item xs={12} md={4}>
                <FormControl fullWidth sx={{ mb: { xs: 1, sm: 2 } }}>
                  ...
                  <Select
                    ...
                    label="Trait"
                    onChange={(e: SelectChangeEvent<string>) => {
                      const trait = (companionTraits as CompanionTrait[]).find(t => t.name === e.target.value);
              ...
              <Paper sx={{ mt: 3, p: { xs: 1, sm: 2 } }}>
                ...
                <TabPanel value={activeTab} index={0}>
                  <Box sx={{ display: 'flex', flexDirection: 'column', gap: 1 }}>
                    <Typography variant="h6" gutterBottom>
                      {builtCompanion.trait.name} ({builtCompanion.size} {builtCompanion.trait.type})
                    </Typography>
                    <Typography variant="body2" color="text.secondary" gutterBottom>
                      Tier {builtCompanion.tier} ({TIER_NAMES[builtCompanion.tier]})
                    </Typography>
                    ...
                    <Paper variant="outlined" sx={{ p: { xs: 1, sm: 2 } }}>
                      <Grid container spacing={1}>
                        <Grid item xs={6} sm={4}>
                          ...
                          <Typography variant="body2">{builtCompanion.calculatedStats.hp}</Typography>
                        </Grid>
                        <Grid item xs={6} sm={4}>
                          <Typography variant="caption" color="text.secondary">AV</Typography>
                          <Typography variant="body2">{builtCompanion.calculatedStats.av}</Typography>
                        </Grid>
                        <Grid item xs={6} sm={4}>
                          <Typography variant="caption" color="text.secondary">Movement</Typography>
                          <Typography variant="body2">{builtCompanion.calculatedStats.movement}</Typography>
                        </Grid>
                      </Grid>
                    </Paper>
                    ...
                    <Grid container spacing={1}>
                      <Grid item xs={12} md={6}>
                        <Paper variant="outlined" sx={{ p: { xs: 1, sm: 2 }, h: '100%' }}>
                          ...
                          <Grid container spacing={1}>
                            {Object.entries(builtCompanion.calculatedStats.attributes).map(([attr, value]) => (
                              <Grid item xs={6} key={attr}>
                                <Typography variant="caption" color="text.secondary">{attr.toUpperCase()}</Typography>
                      ...
                      <Grid item xs={12} md={6}>
                        <Paper variant="outlined" sx={{ p: { xs: 1, sm: 2 }, h: '100%' }}>
                          <Typography variant="subtitle1" gutterBottom>Defenses</Typography>
                          <Grid container spacing={1}>
                            <Grid item xs={4}>
                              <Typography variant="caption" color="text.secondary">Parry</Typography>
                              <Typography variant="body2" fontWeight="bold">{builtCompanion.calculatedStats.defenses.parry}</Typography>
                            </Grid>
                            <Grid item xs={4}>
                              <Typography variant="caption" color="text.secondary">Dodge</Typography>
                              <Typography variant="body2" fontWeight="bold">{builtCompanion.calculatedStats.defenses.dodge}</Typography>
                            </Grid>
                            <Grid item xs={4}>
                              <Typography variant="caption" color="text.secondary">Resist</Typography>
                              <Typography variant="body2" fontWeight="bold">{builtCompanion.calculatedStats.defenses.resist}</Typography>
                            </Grid>
                          </Grid>
                        </Paper>
                      </Grid>
                    ...
                    <Paper variant="outlined" sx={{ p: { xs: 1, sm: 2 }}}>
                      ...
                      <Typography variant="body2">{builtCompanion.calculatedStats.skills}</Typography>
                    </Paper>

                    {/* Attacks */}
                    {builtCompanion.calculatedStats.attacks.length > 0 && (
                      <Paper variant="outlined" sx={{ p: { xs: 1, sm: 2 } }}>
                        <Typography variant="subtitle1" gutterBottom>Attacks</Typography>
                        {builtCompanion.calculatedStats.attacks.map((attack, index) => (
                          <Typography key={index} variant="body2" sx={{ mb: 1 }} dangerouslySetInnerHTML={{ __html: attack }} />
                        ))}
                      </Paper>
                    )}
                    ...
                    {builtCompanion.calculatedStats.abilities.length > 0 && (
                      <Paper variant="outlined" sx={{ p: { xs: 1, sm: 2 } }}>
                        <Typography variant="subtitle1" gutterBottom>Abilities</Typography>
                        {builtCompanion.calculatedStats.abilities.map((ability, index) => (
                          <Typography key={index} variant="body2" sx={{ mb: 1 }} dangerouslySetInnerHTML={{ __html: ability }} />
                        ))}
                      </Paper>
                    )}
                    ...
                    <Paper variant="outlined" sx={{ p: { xs: 1, sm: 2 } }}>
                      <Grid container spacing={1}>
                        <Grid item xs={12} md={4}>
                          <Typography variant="caption" color="text.secondary" gutterBottom>Immunities</Typography>
                          <Typography variant="body2">{builtCompanion.calculatedStats.immunities}</Typography>
                        </Grid>
                        <Grid item xs={12} md={4}>
                          <Typography variant="caption" color="text.secondary" gutterBottom>Resistances</Typography>
                          <Typography variant="body2">{builtCompanion.calculatedStats.resistances}</Typography>
                        </Grid>
                        <Grid item xs={12} md={4}>
                          <Typography variant="caption" color="text.secondary" gutterBottom>Weaknesses</Typography>
                          <Typography variant="body2">{builtCompanion.calculatedStats.weaknesses}</Typography>
                        </Grid>
                      </Grid>
                    </Paper>
                  </Box>
                ...
                <TabPanel value={activeTab} index={1}>
                  <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 2 }}>
                    ...
                    <Button 
                      size="small" 
                      onClick={() => navigator.clipboard.writeText(generateMarkdown(builtCompanion))}
                    >
                      Copy to Clipboard
                    </Button>
                  </Box>
                ...
                <TabPanel value={activeTab} index={2}>
                  <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 2 }}>
                    ...
                    <Button 
                      size="small" 
                      onClick={() => navigator.clipboard.writeText(generateJSON(builtCompanion))}
                    >
                      Copy to Clipboard
                    </Button>
                  </Box>
