	...
	describe('Character CRUD Operations', () => {
		describe('setCharacter', () => {
			...
			it('should fix non-array statusEffects', () => {
				...
				expect(
					Array.isArray(state.activeCharacter!.statistics.statusEffects),
				).toBe(true)
				expectArrayLength(
					state.activeCharacter!.statistics.statusEffects,
					0,
		...
		describe('updateCharacter', () => {
			it('should update character with deep merge', () => {
				...
				expect(state.activeCharacter!.personal.playerName).toBe(
					character.personal.playerName,
				)
				expect(state.activeCharacter!.statistics.health).toEqual(
			...
			it('should use deep copy to prevent mutation', () => {
				...
				expect(state.activeCharacter!.personal.allies).not.toBe(
					update.personal.allies,
				)
				expect(state.activeCharacter!.personal.allies).toEqual(
					update.personal.allies,
				)
			})
	...
	describe('Skills Management', () => {
		...
		describe('updateSkill', () => {
			...
			it('should not affect other skills', () => {
				...
				expect(state.activeCharacter!.skills.skills[1]).toEqual(
					characterWithSkills.skills.skills[1],
				)
			})
		})
		...
		describe('Professions Management', () => {
			it('should add profession', () => {
				...
				expect(state.activeCharacter!.skills.professions).toContain(
					'Blacksmith',
				)
				expectUnsavedChanges(state)
			})
			...
			it('should remove profession', () => {
				...
				expect(state.activeCharacter!.skills.professions).not.toContain(
					'Soldier',
				)
				expect(state.activeCharacter!.skills.professions).toContain(
					'Blacksmith',
		...
		describe('Languages Management', () => {
			...
			it('should remove language', () => {
				characterWithSkills.skills.languages = [
					'Tradespeak',
					'Common',
					'Elvish',
				]
	...
	describe('Abilities Management', () => {
		...
		describe('importAbilities', () => {
			it('should import multiple abilities', () => {
				...
				expect(state.activeCharacter!.skills.abilities[1].title).toBe(
					'Imported 2',
				)
				expectUnsavedChanges(state)
			})
	...
	describe('Weapons Management', () => {
		...
		describe('importWeapons', () => {
			it('should import multiple weapons', () => {
				...
				expect(state.activeCharacter!.items.weapons[1].name).toBe(
					'Imported Axe',
				)
				expectUnsavedChanges(state)
			})
	...
	describe('Items Management', () => {
		...
		describe('importItems', () => {
			it('should import multiple items', () => {
				...
				expect(state.activeCharacter!.items.items[1].name).toBe(
					'Imported Item 2',
				)
				expectUnsavedChanges(state)
			})
	...
	describe('Spells Management', () => {
		...
		describe('importSpells', () => {
			it('should import multiple spells', () => {
				...
				expect(state.activeCharacter!.spells.spells[1].name).toBe(
					'Imported Spell 2',
				)
				expectUnsavedChanges(state)
			})
